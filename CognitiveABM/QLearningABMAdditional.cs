...
namespace CognitiveABM.QLearningABMAdditional{
  ...
  public class QLearningABMAdditional{
     ...
     * choices made in later in episode have less of an affect on the return
     */
    ...
    */
    ...
    public Dictionary<int, float> getAgentScore(List<int> animalIdList, Dictionary<int,List<float[]>> patchDict, float lambda){
      ...
      foreach (int id in animalIdList){ //adds animals last tick score to the dictionary
        patchList = patchDict[id]; //houses information of patches
        scoreValue.Add(id, patchList.Last()[15]);  //Total Fitness Column in landscape_exportInfo.csv
      ...
      scoreValue = setScoreValue(scoreValue); //gradiant of scores from 1 to -1 
    ...
    public void updateQMap(Dictionary<int, float> agentScore, Dictionary<int, List<(int,int)>> agentQmapPath, List<int> animalIdList){
      ...
      foreach(int id in animalIdList){
        ...
        qmap = normalliseQMap(qmap); //columns should add up to 1
      }

      qmap = roundQMap(qmap); //Rounds all the values to 3 places
      printNewQMap(qmap);
    }//end updateQMap
    ...
    public float[,] normalliseQMap(float[,] qmap){
      ...
      float[] colVals = new float[qmap.GetLength(0)];
      ...
      for (int col = 0; col < qmap.GetLength(1); col++){
        ...
        for (int row = 0; row < qmap.GetLength(0); row++){ //should all add up to 1
          normallizedQMap[row,col] = colVals[row]/total;
        }
        total = 0.0f;
