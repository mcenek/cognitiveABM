    ...
    public class Animal : Mars.Interfaces.Agent.IMarsDslAgent
    {
        ...
        public Tuple<float[,], float, float> setLandscapeMinMax(float[,] landscapePatch, float min, float max, float[] elevationArr)
        {
            ...
            for (int x = 0; x < 3; x++) //Getting patch values and turning it into a matrix
            {
                ...
                {
                    if (useDistantView)
                    {//set landscape to distantTerrainElevations + 10*rewards
                    ...
                    {//set landscape to adjacentTerrainElevations + 10*rewards
                        if (elevationArr[index] < min)
                        {
                            min = elevationArr[index];
                        }
        ...
        public void pickUpReward(int xPos, int yPos)
        {
            ...
            {//if no items in dictionary, safely TryAdd
                List<(int, int)> tempList = new List<(int, int)>();
                tempList.Add((xPos, yPos));
                rewardMemory.TryAdd(AnimalId, tempList);
            }
            ...
            {
                List<(int, int)> tempList = rewardMemory[AnimalId];
                tempList.Add((xPos, yPos));
                rewardMemory.TryUpdate(AnimalId, tempList, rewardMemory[AnimalId]);
            }
        }

        //checks if current location contains an active reward
        public bool isOnActiveReward(int xPos, int yPos)
        {
            if (rewardMemory.ContainsKey(AnimalId))
            {
                foreach ((int, int) coord in rewardMemory[AnimalId])
                {
        ...
        public int calculateBioEnergy(bool stayPut, bool onActiveReward)
        {
            int BioEnergy = 0;
        ...
        private Tuple<List<float>, List<float>> GetDistantTerrainInfo()
        {
            ...
            for (int dy = 1; dy >= -1; --dy)
            {
                for (int dx = -1; dx <= 1; ++dx)
                {
                    ...
                    { //if the newX value is out of bounds use adjacent view
                        newX = dx + x;
                    }
        ...
        */
        ...
        {
            if (rewardMemory.ContainsKey(AnimalId) == false)
            {
                return rewards;
            }
            int i = 0;
            ...
            for (i = 0; i < rewardMemory[AnimalId].Count; i++)
            {
                for (j = 0; j < temp.Count; j++)
                {
                    if (temp[j] == rewardMemory[AnimalId][i])
                    {
                        rewards[j] = 0;
                    }
                }
